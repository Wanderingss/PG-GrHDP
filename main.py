...
if __name__ == '__main__':
    ...
    print(1/(1-TestPolicy.Model.beta))
    ...
    for i in range(20):
        ...
        # 训练目标网络
        accuracy = TestPolicy.MatlabTrainValueFunction(eng, TestPolicy.Model.Signal, TrainReinforcement_signalData, Minaccuracy=0.00000001, Maxepochs=100, MaxAccuracy=0.01, showWindow=1)
        print(accuracy)
        torch.save(TestPolicy.Model.Signal, 'Signal.ckpt')
        
        # 训练值网络
        accuracy = TestPolicy.MatlabTrainValueFunction(eng, TestPolicy.Model.ValueFunction, TrainLoss, Minaccuracy=0.00000001, Maxepochs=100, MaxAccuracy=0.01, showWindow=1)
        print(accuracy)
